...
def save_consolidated():
    """Save consolidated master JSON files."""
    os.makedirs("reports", exist_ok=True)
    if PASS_LIST:
        with open(os.path.join("reports", "pass_targets.json"), "w") as f:
            json.dump(PASS_LIST, f, indent=4)
    if FAIL_LIST:
        with open(os.path.join("reports", "fail_targets.json"), "w") as f:
            json.dump(FAIL_LIST, f, indent=4)
    print(f"\n[*] Saved consolidated: {len(PASS_LIST)} passed, {len(FAIL_LIST)} failed")
...
def signal_handler(sig, frame):
    ...
    if not STARTED_SCANNING:
        print("\n\n[!] Exiting...")
        sys.exit(0)
    print("\n\n[!] Force Stop Detected! Saving collected data...")
    EXIT_FLAG = True
    save_consolidated()
...
def analyze_address(address: str):
    ...
    if len(unique_sigs) >= MIN_SIGS_REQUIRED:
        ...
        filepath = os.path.join("reports", "fail", f"{address}.json")
        FAIL_LIST.append(data)
        print(f"  [-] FAILED: Only {len(unique_sigs)} signatures (need {MIN_SIGS_REQUIRED})")
...
def main():
    ...
    already_done = set()
    for folder in ["reports/pass", "reports/fail"]:
    ...
    fail_count = len([f for f in os.listdir("reports/fail") if f.endswith(".json")]) if os.path.isdir("reports/fail") else 0
