...
def main():
    ...
    targets = []
    if os.path.isfile("reports/pass_targets.json"):
        with open("reports/pass_targets.json", "r") as f:
            data = json.load(f)
            if isinstance(data, dict): targets.append(data)
            elif isinstance(data, list): targets.extend(data)
    if os.path.isdir("reports/pass"):
        for filename in os.listdir("reports/pass"):
            if filename.endswith(".json"):
                with open(os.path.join("reports/pass", filename), "r") as f:
                    data = json.load(f)
